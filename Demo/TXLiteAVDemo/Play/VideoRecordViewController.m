...
-(void)createSpeedBtnS
{
    ...
    CGFloat btnBeginCenterX = (self.view.frame.size.width - BUTTON_SPEED_WIDTH * BUTTON_SPEED_COUNT - BUTTON_SPEED_INTERVAL * (BUTTON_SPEED_COUNT - 1)) / 2 + BUTTON_SPEED_WIDTH / 2;
    for(int i = 0 ; i < BUTTON_SPEED_COUNT ; i ++)
    {
        UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.bounds = CGRectMake(0, 0, BUTTON_SPEED_WIDTH, BUTTON_SPEED_HEIGHT);
        btn.center = CGPointMake(btnBeginCenterX + (BUTTON_SPEED_INTERVAL + BUTTON_SPEED_WIDTH) * i, self.view.frame.size.height - 146);
...
-(void)onBtnSpeedClicked:(UIButton *)btn
{
    ...
    SpeedMode speedMode = (SpeedMode)btn.tag;
...
-(void)refreshRecordTime:(NSInteger)second
{
    _currentRecordTime = second;
    [_progressView update:(float)_currentRecordTime / MAX_RECORD_TIME];
